...
int classify(const struct immagine *image, struct immagine *list_images){
	 
  long int mindistance ; //calcolo la distanza tra un'immagine della lista e quella da analizzare
  int corresponding_label; //creo una variabile che conterrà il label dell'immagine fornita in input
  ...
  if(distance<mindistance)  //caso in cui trovo un'immagine più affine a quella da analizzare
  {
   ...
   corresponding_label=list_images->label;}//salvo l'indirizzo
